...
function ProfileIdoClaimableSliderCard({key,index,Ido}:any) {
    ...
    const claim=async(LaunchPoolAddress:any)=>{
        const contract= await sdk?.getContractFromAbi(LaunchPoolAddress,IDO_ABI);
        try{
    ...
    <CardHeader className='flex flex-col items-start gap-10'>
        {
            ...
            <Button onPress={()=>{
                claim(Ido?.LaunchPoolAddress);
            }} color='primary'>Claim</Button>
            <Button  onPress={()=>{
...
export default ProfileIdoClaimableSliderCard